...
## 미등록 제조사 자료 등록 (`/new`)
- 카톡에서 `<자료> 원일산업`처럼 물었는데 자료실에 없는 제조사면, 챗봇이 "📤 자료 등록하기" 버튼(`/new?name=원일산업&t=토큰`)을 줍니다.
- 연동 사용자가 그 페이지에서 파일을 올리면 `pending`으로 접수되고, `/admin`에서 **승인 시 제조사가 새로 생성**되며 문서도 함께 등록됩니다.
- 관리 목록에는 미등록 제조사가 `(신규) 원일산업`으로 표시됩니다.

## 업로드 용량 / 압축 / 링크
- 직접 업로드 파일은 **최대 50MB**.
- **이미지(사진)는 자동 압축**: 긴 변 2200px로 축소 + JPEG 재인코딩(사업자등록증 등 폰 사진에 유용).
- PDF는 서버 자동 압축이 없어요(무료 호스팅 제약). 50MB를 넘는 큰 PDF는 각 업로드 폼의 **"파일 링크(URL)"** 칸에 구글드라이브·드롭박스 공유 링크를 붙이면 용량 무관하게 등록됩니다.
